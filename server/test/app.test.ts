...
it('stores private documents, enforces capabilities and carries compatible history on retries', async () => {
  ...
  const preview = await request(`${path}?preview=1`, user)
  ...
  expect((await request(`${path}?preview=1`, await session())).status).toBe(404)
  const textPreview = await request(`documents/${saved.documents[1].id}?preview=1`, user)
  expect(textPreview.headers.get('content-type')).toBe('application/octet-stream')
