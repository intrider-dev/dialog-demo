...
Ответы отображаются с Markdown: заголовками, выделением, списками, цитатами и таблицами.
У блоков кода есть кнопка копирования. Длинные строки и таблицы прокручиваются внутри
ответа. HTML из ответа не выполняется, внешние картинки не загружаются автоматически.
...
Клик по картинке в сообщении открывает увеличенный просмотр через Dialog из shadcn/ui.
Закрыть его можно крестиком, клавишей Escape или кликом на фон.
Можно прикрепить **3 изображения**. Большие файлы автоматически подготавливаются
в браузере: до 2048 пикселей по длинной стороне и 2 МБ на изображение, без обрезки.
Сначала используется сжатие без потерь, затем качество 95-85%, при необходимости
дополнительно уменьшается разрешение. Принимаются форматы, которые декодирует браузер,
включая PNG, JPEG, WebP, GIF (первый кадр), BMP и AVIF. Для неподдерживаемых форматов
предлагается сохранить файл как PNG или JPEG. Единого лимита для всех моделей нет;
используется общий профиль подготовки, ошибки провайдера отображаются пользователю.
Можно отправить изображение без текста. Пока изображения прикреплены, текстовые модели
...
проверяет содержимое, удаляет метаданные и ограничивает размер 2048 пикселями
по длинной стороне. WebP сохраняется без потерь, если помещается в 2 МБ;
иначе качество снижается постепенно. В контекст попадает до 3 последних изображений, включая новые;
при выборе текстовой модели передаётся только текст истории.

Документы: **PDF, TXT, MD, CSV и JSON**, до **3 файлов по 5 МБ**. Текстовые файлы
должны быть в UTF-8 и содержать не более **20 000 символов** каждый. Содержимое не обрезается.
PDF передаётся целиком моделям с поддержкой файлов; текстовые документы доступны всем моделям.
В списке возможности обозначены иконками с подсказками. При прикреплённом PDF несовместимые
модели недоступны. Другие форматы нужно предварительно сохранить в PDF.
Документы сохраняются в PostgreSQL и скачиваются только в своей сессии. В контекст включаются
до 3 последних документов с учётом новых; PDF из истории передаются только совместимой модели.
...
Markdown обрабатывают **react-markdown 10.1.0** и **remark-gfm 4.0.1**.
Таблицы, разделители и кнопки используют **Table, Separator и Button из shadcn/ui**.
...
- `server/src/models.ts`: каталог моделей и поддержка изображений и PDF.
- `server/src/documents.ts`: проверка документов и извлечение текста.
...
последние **20 пар**, не более **24 000 символов** вместе с новым вопросом, без учёта вложений.
