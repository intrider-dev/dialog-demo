...
it('marks image-capable models and prevents switching attachments to a text-only model', async () => {
  ...
  expect(onChange).toHaveBeenCalledWith('vision', true, false)
  await userEvent.click(trigger)
  expect(screen.getByRole('option', { name: /Text/ })).toHaveAttribute('aria-disabled', 'true')
  expect(screen.getByRole('option', { name: /Vision/ })).not.toHaveTextContent('Изображения')
  expect(screen.getByRole('img', { name: 'Изображения' })).toHaveAttribute('title', 'Изображения')
  onChange.mockClear()
